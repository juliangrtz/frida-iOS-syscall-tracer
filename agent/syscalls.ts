...
function formatArguments(syscall, cpuContext) {
...
export function printSyscall(cpuContext) {
    /*
    ...
    */
    ...
    let syscallNumber = cpuContext.x16.toInt32();

    if (syscallNumber < 0 && !Config.logMachSyscalls)
        return;

    let syscall = SYSCALLS[syscallNumber] ?? "Unknown syscall";

    log(`${syscall.name}(${formatArguments(syscall, cpuContext)})`);
...
export const SYSCALLS = {
    ...
    "-10": { name: "_kernelrpc_mach_vm_allocate_trap", ignature: "" },
